...
async function loadBothFromDrive(orders, gi) {
  ...
  try {
    ...
    const itemMap = new Map();
    contents.forEach(content => { content.items.forEach(i => itemMap.set(String(i.item), i)); });
    const merged = {
    ...
    if (gi != null) {
      const subEl = document.getElementById(`sg-sub-${gi}`);
      if (subEl) { const span = subEl.querySelector('.sg-total'); if (span) span.textContent = merged.items.length + ' items'; }
    }
...
function setSpeed(speed) {
  ...
  if (speed === 'warp' && warpBtn) {
    ...
    filter.setAttribute('id', 'bolt-glow'); filter.setAttribute('x', '-80%'); filter.setAttribute('y', '-80%');
    ...
    ['blur','blur','SourceGraphic'].forEach(n => { const node = document.createElementNS(ns, 'feMergeNode'); node.setAttribute('in', n); merge.appendChild(node); });
    ...
    function makePath(d, stroke, width, dashLen, delay) {
      ...
      p.setAttribute('d', d); p.setAttribute('stroke', stroke); p.setAttribute('stroke-width', String(width));
      ...
      const anim = delay ? `boltDraw 0.85s ease forwards ${delay}s,boltFlash 0.85s ease forwards ${delay}s` : 'boltDraw 0.85s ease forwards,boltFlash 0.85s ease forwards';
    ...
    function makeBranch(d, stroke, width, delay) {
      const p = document.createElementNS(ns, 'path');
      p.setAttribute('d', d); p.setAttribute('stroke', stroke); p.setAttribute('stroke-width', String(width));
      p.setAttribute('fill', 'none'); p.setAttribute('stroke-linecap', 'round'); p.setAttribute('filter', 'url(#bolt-glow)');
    ...
    setTimeout(() => { warpBtn.classList.remove('warp-lightning'); svg.remove(); }, 950);
  }
...
function getState() {
  return new Promise(res => {
    ...
    chrome.runtime.sendMessage({ type:'GET_STATE' }, r => { clearTimeout(timer); res(r || { phase: 'idle' }); });
  });
}
...
function _runTimerInterval(start) {
  ...
  timerInterval = setInterval(() => { const el = document.getElementById('timerDisplay'); if (el) el.textContent = _formatElapsed(start); }, 500);
}
function stopTimer() {
...
function resetTimer() {
  ...
  const el = document.getElementById('timerDisplay'); if (el) el.textContent = '0:00';
  const final = document.getElementById('timerFinal'); if (final) final.textContent = '0:00';
}
...
function renderState() {
  ...
  if (localState?.orderData?.store) { store.textContent = storeLabelPlain(localState.orderData.store); store.style.display = 'block'; }
...
function renderPreview() {
  ...
  document.getElementById('previewList').innerHTML = items.map(i => {
    ...
    const orderDisplay = isCases ? absOrder + '<span style="color:var(--accent);font-size:9px;font-weight:700;display:block;line-height:1.2">' + (absOrder===1?'case':'cases') + '</span>' : absOrder;
    return `<div class="preview-row"><div class="pr-item">${i.item}</div><div class="pr-order">${orderDisplay}</div><div class="pr-qoh">${i.qoh}</div></div>`;
  }).join('');
...
async function runBot() {
  ...
  if (!tab || !tab.url.includes('petvalu.com')) {
    ...
    await setState({ phase:'loaded' }); stopPolling(); localState = await getState(); renderState(); return;
  }
  try {
    ...
    await setState({ phase:'loaded' }); stopPolling(); localState = await getState(); renderState();
  }
}

function wireStopBtn() { const btn = document.getElementById('btnStop'); if (btn) btn.onclick = stopBot; }
async function stopBot() {
  await setState({ stopRequested:true }); releaseWakeLock();
...
function renderLog() {
  ...
  if (logs.length < existing) {
    wrap.innerHTML = '';
    logs.forEach(l => { const line = document.createElement('div'); line.className = 'log-line ' + (l.kind||'info'); line.textContent = l.msg; wrap.appendChild(line); });
  } else {
    logs.slice(existing).forEach(l => { const line = document.createElement('div'); line.className = 'log-line ' + (l.kind||'info'); line.textContent = l.msg; wrap.appendChild(line); });
  }
...
function showEmailPrompt() {
  ...
  el.style.display = 'flex'; el.className = 'email-badge';
  el.style.background = 'var(--yd)'; el.style.borderColor = 'rgba(146,64,14,0.3)'; el.style.color = 'var(--yellow)';
  el.style.flexDirection = 'column'; el.style.alignItems = 'flex-start'; el.style.gap = '8px';
...
async function botScript(orderData, timings, runId) {
  ...
  async function setFilter(label, value) {
    ...
    input.focus(); input.value = ''; input.dispatchEvent(new Event('input', {bubbles:true})); await sleep(100);
    input.value = String(value).toUpperCase();
    input.dispatchEvent(new Event('input', {bubbles:true})); input.dispatchEvent(new Event('change', {bubbles:true})); input.dispatchEvent(new KeyboardEvent('keyup', {bubbles:true}));
    return true;
  }
  async function clearFilter(label, forceReal) {
    const input = getFilterInput(label); if (!input) return;
    if (timings.overwriteMode && !forceReal) { input.focus(); input.select(); }
    else { input.value = ''; input.dispatchEvent(new Event('input', {bubbles:true})); input.dispatchEvent(new Event('change', {bubbles:true})); if (timings.afterClear > 0) await sleep(timings.afterClear); }
  }
  ...
  function getCellText(row, colId) {
    let c = row.querySelector(`[col-id="${colId}"]`); if (c) return c.textContent.trim();
  ...
  function getAgApi() {
    try {
      const agGridEl = document.querySelector('ag-grid-angular'); if (!agGridEl) return null;
  ...
  function getRowDataFromGrid(itemId) {
    try {
      const api = getAgApi(); if (!api) return null;
      let found = null;
      api.forEachNodeAfterFilter(node => { if (found) return; const d = node.data; if (d && String(d.item_no||'').trim().toLowerCase() === String(itemId).trim().toLowerCase()) found = d; });
      return found;
    } catch(e) { return null; }
  }
  ...
  async function enterQty(row, qty) {
    ...
    cell.dispatchEvent(new MouseEvent('dblclick', {bubbles:true, cancelable:true, view:window})); await sleep(400);
    let input = cell.querySelector('input[aria-label="Input Editor"]') || cell.querySelector('input') || document.querySelector('.ag-cell-inline-editing input');
    if (!input) { cell.dispatchEvent(new KeyboardEvent('keydown', {key:'F2', keyCode:113, bubbles:true})); await sleep(350); input = cell.querySelector('input') || document.querySelector('.ag-cell-inline-editing input'); }
    if (!input) return false;
    input.focus(); input.select(); input.value = String(qty);
    input.dispatchEvent(new Event('input', {bubbles:true})); input.dispatchEvent(new Event('change', {bubbles:true}));
    input.dispatchEvent(new KeyboardEvent('keydown', {key:'Tab', keyCode:9, bubbles:true})); await sleep(timings.afterQty);
    return true;
  }
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (!targetRow) {
      ...
      rows = getVisibleRows(); targetRow = rows.length > 0 ? rows[0] : null; usedSub = true;
    }
    if (!targetRow) {
...
function devTriggerClick() { _devClicks++; clearTimeout(_devClickTimer); _devClickTimer = setTimeout(() => { _devClicks = 0; }, 1200); if (_devClicks >= 3) { _devClicks = 0; openDevMode(); } }
...
async function devTestUpdate() {
  ...
  try {
    ...
    const latest = match ? match[1] : '?', current = browser.runtime.getManifest().version;
